...
class CardCarousel {
  ...
  calcPos(x, scale) {
    ...
    const centerPos = 100 + spacingFactor * x;
    const adjustment = (scale * 100 + this.cardWidth) / 2;
    let formula = centerPos - adjustment;

    if (window.innerWidth <= 1200) {
      // Adjust for mobile view as needed
      formula = 95 - adjustment + spacingFactor * x;
    }
    if (window.innerWidth <= 991) {
      // Adjust for mobile view as needed
      formula = 70 - adjustment + spacingFactor * x;
    }
    if (window.innerWidth <= 768) {
      // Adjust for mobile view as needed
      formula = 92.5 - adjustment + spacingFactor * x;
    }
    if (window.innerWidth <= 576) {
      // Adjust for mobile view as needed
      formula = 80 - adjustment + spacingFactor * x;
    }
    return formula;
  }
  calcScale(x) {
